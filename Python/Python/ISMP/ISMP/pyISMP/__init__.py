...
def start_end_lines(im_, n2_, m2_, threshold_):
    ...
    numSamples = 2 * max(m2_, n2_) # number of points for creating a line
    ...
    for k in range(len(i1_)):
        p = 0
        end_ = [] #counter
        x1, y1 = j1_[k], m2_ - i1_[k] + 1    #line start point coordinates
        for q in range(len(i_)):
            x2, y2 = j_[q], m2_ - i_[q] + 1    #line end point coordinates
...
def degree_of_node(adj_, m2_, n2_):
    """
    ...
    edge_ = edge.reshape((m2_, n2_)) #Reshape degree of node to image size.
    return edge_

"""Page-Rank"""

def page_rank(G_, m2_, n2_):
    """
    ...
    pr_ = pr_arr[:,1].reshape((m2_, n2_)) # Reshape Page-Rank to image size
    return pr_
...
def run(FitsFile, threshold_=18):
    ...
    threshold_=18
    I = start_end_lines(im, n2, m2, threshold_)  # Extraction of connections
